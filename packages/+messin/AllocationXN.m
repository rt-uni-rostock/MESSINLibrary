function [throttleSB,throttlePS,rudderSB,rudderPS] = AllocationXN(X,N)
    %messin.AllocationXN Solve XN allocation problem for the MESSIN USV.
    % 
    %           .-.                       .-.
    %           | |                       | |
    %           | |                       | |
    ...
    % Explicit form of pseudo-inverse matrix: invB = pinv([1 0 1 0; -y1 x1 -y2 x2]);
    x1 = positionSB(1);
    y1 = positionSB(2);
    x2 = positionPS(1);
    y2 = positionPS(2);
    ...
    y2y2 = y2 * y2;
    y1y2 = y1 * y2;
    den = x1x1+x1x1 + x2x2+x2x2 + y1y1 - 2.0*y1y2 + y2y2;
    invB = [0.5 -0.25; 0.0 -0.25; 0.5 0.25; 0.0 -0.25];
    if(abs(den) > 0.0)
        tmp1 = x1x1 + x2x2 + y1y1 + y2y2 - y1y2;
        invB = [
            tmp1-y1y1,       y1+y2-y1-y1;
            x1*(y1+y2),        x1+x1;
            tmp1-y2y2,       y1+y2-y2-y2;
            x2*(y1+y2),        x2+x2
        ] ./ den;
    end
